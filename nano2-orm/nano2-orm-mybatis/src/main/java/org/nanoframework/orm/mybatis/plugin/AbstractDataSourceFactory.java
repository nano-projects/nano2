...
public class AbstractDataSourceFactory implements DataSourceFactory {
    ...
    public void setProperties(Properties properties) {
        ...
        var metaDataSource = SystemMetaObject.forObject(dataSource);
        for (var key : properties.keySet()) {
            var propertyName = (String) key;
            if (metaDataSource.hasSetter(propertyName)) {
                var value = (String) properties.get(propertyName);
                /** 对没有设置值的属性跳过设置 */
                if (StringUtils.isNotEmpty(value) && value.startsWith("${") && value.endsWith("}")) {
                    continue;
                }

                var convertedValue = convertValue(metaDataSource, propertyName, value);
    ...
    private Object convertValue(MetaObject metaDataSource, String propertyName, String value) {
        ...
        var targetType = metaDataSource.getSetterType(propertyName);
