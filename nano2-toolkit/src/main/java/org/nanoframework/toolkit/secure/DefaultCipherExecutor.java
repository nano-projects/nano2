 ...
 * @since 1.3.7
 */
public class DefaultCipherExecutor implements CipherExecutor {
    private static final Logger LOGGER = LoggerFactory.getLogger(DefaultCipherExecutor.class);
     ...
     * <p>
     * Note that in order to customize the encryption algorithms, you will need to download and install the JCE
     * Unlimited Strength Jurisdiction Policy File into your Java installation.
     * </p>
    ...
    public DefaultCipherExecutor(String secretKeyEncryption, String secretKeySigning) {
        this(secretKeyEncryption, secretKeySigning, ContentEncryptionAlgorithmIdentifiers.AES_128_CBC_HMAC_SHA_256,
     ...
     */
    public DefaultCipherExecutor(String secretKeyEncryption, String secretKeySigning,
            String contentEncryptionAlgorithmIdentifier, String signingAlgorithm) {
        ...
        LOGGER.debug("Initialized cipher signing sequence via [{}]", signingAlgorithm);

    }

    @Override
    public String encode(@NonNull String value) {
        return signValue(encryptValue(value));
    }

    @Override
    public String decode(@NonNull String value) {
        var encoded = verifySignature(value);
    ...
    private Key prepareJsonWebTokenKey(String secret) {
        try {
            return JsonWebKey.Factory.newJwk(Map.of("kty", "oct", "k", secret)).getKey();
        } catch (Exception e) {
    ...
    /**
     * Encrypt the value based on the seed array whose length was given during init, and the key and content encryption
     * ids.
     * @param value the value
     * @return the encoded value
     */
    private String encryptValue(@NonNull String value) {
        try {
            ...
            throw new RuntimeException(
                    "Ensure that you have installed JCE Unlimited Strength Jurisdiction Policy Files. "
                            + e.getMessage(),
                    e);
        }
    }
     ...
     */
    private String decryptValue(@NonNull String value) {
        try {
            var jwe = new JsonWebEncryption();
            jwe.setKey(this.secretKeyEncryptionKey);
     ...
     */
    private String signValue(@NonNull String value) {
        try {
     ...
     */
    private String verifySignature(@NonNull String value) {
        try {
            var jws = new JsonWebSignature();
            jws.setCompactSerialization(value);
            jws.setKey(this.secretKeySigningKey);
            var verified = jws.verifySignature();
            if (verified) {
