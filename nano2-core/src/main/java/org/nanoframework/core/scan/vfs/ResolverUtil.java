...
public class ResolverUtil<T> {
     ...
     * @return Attempts to discover classes that are assignable to the type provided
     */
    ...
    /**
     * Attempts to discover classes that are annotated with the annotation. Accumulated classes can be accessed by
     * calling {@link #getClasses()}.
     * @param annotation the annotation that should be present on matching classes
     * @param packageNames one or more package names to scan (including subpackages) for classes
     * @return Attempts to discover classes that are annotated with the annotation
     */
    public ResolverUtil<T> findAnnotated(Class<? extends Annotation> annotation, String... packageNames) {
        ...
        var test = new AnnotatedWith(annotation);
        for (var pkg : packageNames) {
            find(test, pkg);
        }
     ...
     * @return Scans for classes starting at the package provided and descending into subpackages
     */
    ...
    protected void addIfMatching(Test test, String fqn) {
        try {
            var externalName = fqn.substring(0, fqn.indexOf('.')).replace('/', '.');
            var loader = getClassLoader();
            LOGGER.debug("Checking to see if class " + externalName + " matches criteria [" + test + ']');

            var type = loader.loadClass(externalName);
            if (test.matches(type)) {
                matches.add((Class<T>) type);
            }
        } catch (Throwable t) {
            LOGGER.warn("Could not examine class '" + fqn + '\'' + " due to a " + t.getClass().getName()
